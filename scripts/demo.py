...
def show_platform():
    ...
    for r in stats[stats["category"].isin(["chicken breast", "milk", "spinach", "rice"])].itertuples(index=False):
        print(f"    {r.category:<16} ${r.min_price:>5.2f}-${r.max_price:<5.2f}  cheapest at {r.cheapest_store}")
...
def main():
    ...
    pf.weights = Weights(price_weight=1.0, distance_weight=0.05, substitution_penalty=2.0,
                         store_visit_penalty=0.30, coverage_penalty=25.0)
