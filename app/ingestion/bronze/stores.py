...
CHAINS: list[dict] = [
    {"name": "ValueMart", "base_mult": 0.95, "stock_p": 0.95, "default_coverage": 0.90,
     "cat_mult": {}, "cat_coverage": {}},
    {"name": "MegaSaver", "base_mult": 0.92, "stock_p": 0.94, "default_coverage": 0.88,
     "cat_mult": {"grains": 0.78, "bakery": 0.85, "breakfast": 0.80, "beverages": 0.82,
    ...
    {"name": "GreenBasket", "base_mult": 1.10, "stock_p": 0.95, "default_coverage": 0.35,
     "cat_mult": {"produce": 0.72, "dairy": 0.90}, "cat_coverage": {"produce": 0.95, "dairy": 0.90}},
...
def model(seed: int = 42, n_stores: int = 8, n_days: int = 2,
          max_products_per_store: int = 120) -> None:
    ...
    for s in range(n_stores):
        ...
        stores.append({"store_id": sid, "name": f"{chain['name']} #{s+1}",
                       ...
                       "ingested_at": "2024-01-01T06:00:00", "source_system": "modeled_pricing"})
        kept = 0
    ...
    for d in range(n_days):
        ...
        for (sid, bc), (chain, base, grp) in carries.items():
            ...
            events.append({"event_id": f"e{eid:07d}", "store_id": sid, "barcode": bc,
                           "price": round(price, 2), "in_stock": rng.random() < chain["stock_p"],
                           "observed_at": observed, "source_system": "modeled_pricing"})
            eid += 1
    for i in range(n_days):
        day = f"2024-01-{i+1:02d}"
        batch = [e for e in events if e["observed_at"].startswith(day)]
        io.write_delta(pd.DataFrame(batch), paths.BRONZE_RAW_PRICE_EVENTS,
