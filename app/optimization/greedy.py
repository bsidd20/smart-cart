...
def _plan_metrics(
    chosen: dict, missing: list, options: dict, qty: dict, oneway_km: dict, w: Weights
) -> dict:
    lines = [
        Line(item, qty[item], sid, options[item][sid]["unit_price"], options[item][sid]["score"])
...
def _build_result(
    ...
    for item, sid in chosen.items():
        ...
        reason = (
            f"${opt['unit_price']:.2f} at {store.name} - matched "
        ...
        assignments.append(
            Assignment(
                ...
                unit_price=round(opt["unit_price"], 2),
                line_cost=round(qty[item] * opt["unit_price"], 2),
                match_score=opt["score"],
                match_method=opt["method"],
                reason=reason,
            )
        )
...
def optimize_single_store(options, qty, oneway_km, repo, w) -> OptimizationResult:
    items = list(options)
    best_metrics, best_chosen, best_missing = None, None, None
    for sid in repo.all_store_ids():
        chosen, missing = {}, []
        for item in items:
            (chosen.__setitem__(item, sid) if sid in options[item] else missing.append(item))
    ...
    return _build_result(
        "single_store", best_chosen, best_missing, options, qty, oneway_km, repo, w, len(items)
    )
...
def optimize_cart(
    repo: Repository, matcher: ProductMatcher, cart: list[UserCartItem], settings: Settings
) -> dict:
    options = _collect_options(matcher, cart)
    qty = {ci.raw_text: ci.quantity for ci in cart}
    oneway_km = {
        sid: repo.distance_km(sid, settings.user_lat, settings.user_lon)
        for sid in repo.all_store_ids()
    }
    w = settings.weights
