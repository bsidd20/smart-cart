...
def optimize_multi_store_ilp(options, qty, oneway_km, repo: Repository,
                             w: Weights, max_stores: int) -> OptimizationResult | None:
    try:
    ...
    for (i, s), var in x.items():
        cost = (w.price_weight * qty[i] * options[i][s]["unit_price"]
                + w.substitution_penalty * (1 - options[i][s]["score"]))
    ...
    result = _build_result("multi_store", chosen, missing, options,
